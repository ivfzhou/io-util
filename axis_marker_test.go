...
func TestAxisMarker(t *testing.T) {
	t.Run("随机长度写入，每个点只写入一次", func(t *testing.T) {
		for i := 0; i < 100; i++ {
			axis := make([]int, 1024*1024*50*(rand.Intn(5)+1)+10)
			parts := Split(axis)
			...
			expectedResult := fmt.Sprintf("{[0,%d)}", len(axis))
	...
	t.Run("随机长度写入，有些点多次写入", func(t *testing.T) {
		for i := 0; i < 100; i++ {
			axis := make([]int, 1024*1024*50*(rand.Intn(5)+1)+10)
			parts := Split(axis)

			offset := rand.Intn(len(axis))
			parts2 := Split(axis[offset:])
