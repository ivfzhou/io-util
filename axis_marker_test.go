...
func TestAxisMarker_Mark(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m.Mark(tt.markOffset, tt.markLength)
			if m.String() != tt.markResult {
				t.Errorf("AxisMarker.Mark() returns %s, want %s", m.String(), tt.markResult)
			}
		})
	}
}
...
func TestAxisMarker_Get(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != tt.getResult {
				t.Errorf("AxisMarker.Get() returns %d, want %d", result, tt.getResult)
			}
		})
	}
}
...
func TestAxisMarker(t *testing.T) {
	t.Run("随机长度写入，每个点只写入一次", func(t *testing.T) {
		for i := 0; i < 100; i++ {
			axis := make([]int, 1024*1024*100*(rand.Intn(5)+1)+10)

			type part struct {
				offset, length int
			}
			parts := make([]*part, 0, 10)
			var fn func(arr []int, offset int)
			fn = func(arr []int, offset int) {
				if len(arr) <= 0 {
					return
				}
			...
			for _, v := range parts {
				axisMarker.Mark(v.offset, v.length)
			}
			expected := fmt.Sprintf("{[0,%d)}", len(axis))
			if axisMarker.String() != expected {
				t.Errorf("AxisMarker.Mark() returns %s, want %s", axisMarker.String(), expected)
			}
		}
	})

	t.Run("随机长度写入，每个点可能多次写入", func(t *testing.T) {
		for i := 0; i < 100; i++ {
			axis := make([]int, 1024*1024*100*(rand.Intn(5)+1)+10)
			...
			fn(axis, 0)
			axis2 := make([]int, len(axis)/(rand.Intn(3)+1))
			offset := rand.Intn(max(len(axis)-len(axis2), 1))
			fn(axis2, offset)
...
func newCoordinateAxisMarker(t *testing.T) *iu.AxisMarker {
	...
	marker.Mark(10, 20)
	result := "{[10,30)}"
	if marker.String() != result {
		t.Errorf("marker.String() returns %s, want %s", marker.String(), result)
	}
